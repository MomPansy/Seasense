...
export interface VesselDetails extends Vessel {
  modelNumber: string;
  lastPorts: Array<{ port: string; country: string }>;
  groupOwner: string;
...
const vesselNames = [
  'MAERSK ESSEX', 'OCEAN PRINCESS', 'ATLANTIC VOYAGER', 'PACIFIC NAVIGATOR',
  'CARIBBEAN EXPLORER', 'NORDIC STAR', 'ASIAN PHOENIX', 'MEDITERRANEAN QUEEN',
  'ARABIAN PRIDE', 'BALTIC SPIRIT', 'INDIAN VENTURE', 'ARCTIC MONARCH',
  'AUSTRAL EMPEROR', 'COASTAL CHAMPION', 'DEEP SEA GUARDIAN', 'EMERALD DYNASTY',
  'FREEDOM FORTUNE', 'GLOBAL HARMONY', 'HORIZON SEEKER', 'IMPERIAL GLORY',
  'JADE MARINER', 'KINGDOM TRADER', 'LIBERTY WAVE', 'MARITIME LEGEND',
  'NEPTUNE CROWN', 'ODYSSEY MASTER', 'PEARL CARRIER', 'QUANTUM SAILOR',
];

const groupOwners = [
  'Maersk Line', 'Ocean Network Express', 'Mediterranean Shipping Company',
  'CMA CGM Group', 'Hapag-Lloyd', 'Evergreen Marine', 'COSCO Shipping',
  'Yang Ming Marine', 'Hyundai Merchant Marine', 'Pacific International Lines'
];

const countries = [
  'Denmark', 'Japan', 'Switzerland', 'France', 'Germany', 'Taiwan',
  'China', 'South Korea', 'Singapore', 'Panama', 'Liberia', 'Marshall Islands'
];
...
function getRandomPorts(count: number): Array<{ port: string; country: string }> {
  const shuffled = [...ports].sort(() => 0.5 - Math.random());
  return shuffled.slice(0, count);
}

function generateThreatParameters(vesselType: string, threatLevel: number): string[] {
...
export function generateMockVessels(count: number = 30): Vessel[] {
  const vessels: Vessel[] = [];
  
  for (let i = 0; i < count; i++) {
    vessels.push({
      id: `vessel-${i + 1}`,
      name: vesselNames[i % vesselNames.length] + (i >= vesselNames.length ? ` ${Math.floor(i / vesselNames.length) + 1}` : ''),
...
export function generateDetailedVessel(vessel: Vessel): VesselDetails {
  const threatPercentage = 10 + (vessel.threatLevel * 15) + Math.floor(Math.random() * 10);
