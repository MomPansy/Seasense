...
class VesselArrivalsTransformer:
    ...
    def staging_transform_row(self, fetched_at, response_json):
        ...
        for record in response_json:
            ...
            raw_arrival = record.get("arrivedTime")
            if raw_arrival:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                INSERT INTO staging.{self.data_name} (
                    ...
                    arrived_time, location_from, location_to,
                    fetched_at
                )
                VALUES (
                    %s,%s,%s,%s,
                ...
                item['arrived_time'], item['location_from'], item['location_to'],
                item['fetched_at'])
            )
    ...
    def transform(self, conn):
        with conn.cursor() as cur:
            # pick recent raw rows (or use flags/processed_at to avoid reprocessing)
            cur.execute(f"""
